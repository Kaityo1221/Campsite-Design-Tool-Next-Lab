...
3. Creative Mode入口をKMZ中心に整理
   - 通常UIの「KMZ / KMLを選択」を「KMZを選択」へ変更する。
   - 案内文を「編集するKMZを選択してください。」へ変更する。
   - KML対応を内部に残す場合でも、通常ユーザー向け入口では積極的に見せない。

4. 誤ファイル形式の案内強化
   - `.mht` / `.mhtml` / `.eml` など、Google My MapsのWebページ保存ファイルを識別できる場合は専用メッセージを出す。
   - 「読み込めません」だけで終わらせず、「Google My MapsからKMZを書き出して読み込んでください」と案内する。
   - My Mapsを開く安全な導線を作れる場合は追加検討する。
...
安全運用ルールに従い、上記を一括実装せず1機能ずつコミット・実機確認して積み上げる。mainへのマージは明示指示があるまで行わない。
